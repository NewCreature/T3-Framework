...
void dot_intro_render(void)
{
	...
	for(i = 0; i < 480 / 16; i++)
	{
		al_draw_bitmap(dot_bitmap[DOT_BITMAP_BALL_RED + i % 6], i * 16, sin((float)(i * 2 + dot_tick) / 10.0) * 32 + 64, 0);
		al_draw_bitmap(dot_bitmap[DOT_BITMAP_BALL_RED + i % 6], i * 16, cos((float)(i * 2 + dot_tick) / 10.0) * 32 + 480 - 64 - 16 - 1, 0);
	}
	dot_game_render_hud();
