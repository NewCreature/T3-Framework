...
remake_dir _build
cd _build
cmake .. -DPNG_EXECUTABLES=OFF -DPNG_SHARED=OFF -DPNG_TESTS=OFF -DZLIB_INCLUDE_DIR=/usr/local/include -DZLIB_LIBRARY_RELEASE=/usr/local/lib/libz.a -DPNG_BUILD_ZLIB=ON -DCMAKE_INSTALL_PREFIX=/mingw32 -G "Unix Makefiles"
make
make install
cd ..
cd ..
