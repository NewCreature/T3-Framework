...
bool t3f_update_animation(T3F_ANIMATION * ap)
{
	...
	{
		ap->data = t3f_fetch_object(ap->object_loader);
		if(ap->data)
		{
			_t3f_fix_animation_frames(ap);
			for(i = 0; i < ap->data->bitmaps->count; i++)
...
static void _t3f_draw_scaled_rotated_bitmap(ALLEGRO_BITMAP * bp, ALLEGRO_COLOR color, float cx, float cy, float x, float y, float z, float angle, float scale_x, float scale_y, int flags)
{
...
static void _t3f_draw_scaled_bitmap(ALLEGRO_BITMAP * bp, ALLEGRO_COLOR color, float x, float y, float z, float w, float h, int flags)
{
	/* upper left and bottom right points in 3d */
