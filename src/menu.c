...
void t3f_set_menu_item_flags(ALLEGRO_MENU * menu, int item, int flags)
{
    int old_flags = al_get_menu_item_flags(menu, item) & ~ALLEGRO_MENU_ITEM_CHECKBOX;
    if(flags != old_flags)
    {
        al_set_menu_item_flags(menu, item, flags);
    }
}
